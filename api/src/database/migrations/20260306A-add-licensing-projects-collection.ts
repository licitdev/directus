...
export async function up(knex: Knex): Promise<void> {
	...
	const settings = await knex('directus_settings')
		.select('project_id', 'project_name', 'project_url')
		.first();
	...
	if (settings?.project_id) {
		const projectName =
			typeof settings.project_name === 'string' && settings.project_name.trim()
				...
				: 'Directus Project';
		const publicUrl =
			typeof settings.project_url === 'string' && settings.project_url.trim()
				? settings.project_url.trim()
				: null;
