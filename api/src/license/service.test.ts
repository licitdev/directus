...
test('constructor throws when LICENSING_SERVICE_URL is missing', () => {
	...
	expect(() => new LicensingService()).toThrow(
		'LICENSING_SERVICE_URL environment variable is required',
	);
});
...
test('verify POSTs to /v1/verify with request body', async () => {
	...
	await service.verify({
		...
		public_url: 'https://my-directus.example.com',
	});
...
test('verify throws when response is not ok', async () => {
	...
	).rejects.toThrow('License verification failed: 403 Forbidden. Invalid license');
});
...
test('verify throws when response has no token', async () => {
	...
	).rejects.toThrow('Invalid license response: missing or invalid license_token');
});
...
test('verify throws when persistence updates zero rows', async () => {
	...
	).rejects.toThrow('Failed to persist license_token: no directus_settings row updated');
});
